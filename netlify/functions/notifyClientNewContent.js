...
export const handler = async (event) => {
  ...
  try {
    ...
    if (!data?.project_id) {
      return { statusCode: 200, body: JSON.stringify({ skipped: 'no project_id' }) };
    }
    if (data.category !== 'deliverables') {
      return { statusCode: 200, body: JSON.stringify({ skipped: 'not a deliverable' }) };
    }
    ...
    if (data.created_by && data.created_by !== project.owner_email) {
      return { statusCode: 200, body: JSON.stringify({ skipped: 'uploaded by client, not filmmaker' }) };
    }

    const clientEmail = project.client_email;
    if (!clientEmail) return { statusCode: 200, body: JSON.stringify({ skipped: 'no client email' }) };
