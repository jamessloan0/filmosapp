...
export const handler = async (event) => {
  ...
  try {
    ...
    if (!entityName || !eventType || !data) {
      return { statusCode: 200, body: JSON.stringify({ skipped: 'missing payload' }) };
    }
    ...
    if (entityName === 'Message' && eventType === 'create') {
      if (data.sender_type !== 'client') {
        return { statusCode: 200, body: JSON.stringify({ skipped: 'filmmaker message' }) };
      }
      const project = await getProject(data.project_id);
      if (!project) return { statusCode: 200, body: JSON.stringify({ skipped: 'project not found' }) };
    ...
    // Feedback decision changed
      ...
      if (!wasChanged) return { statusCode: 200, body: JSON.stringify({ skipped: 'no decision change' }) };
    ...
    // Proposal status changed by client
      ...
      if (!wasClientResponse) return { statusCode: 200, body: JSON.stringify({ skipped: 'not client response' }) };
      ...
      return { statusCode: 200, body: JSON.stringify({ skipped: `unhandled: ${entityName} ${eventType}` }) };
    }
